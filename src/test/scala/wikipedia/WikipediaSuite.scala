...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'rankLangs' should work for RDD with similar languages like Java and JavaScript") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("JavaScript", "Java")
    val rdd = sc.parallelize(List(WikipediaArticle("1", "JavaScript is great"), WikipediaArticle("2", "Java is OK, but JavaScript is cooler")))
    val ranked = rankLangs(langs, rdd)
    val res = ranked.head._1 == "JavaScript"
    val occurences = ranked.head._2 == 2
    assert(res)
    assert(occurences)
  }
